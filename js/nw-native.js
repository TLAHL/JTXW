...
(function (window, undefined) {
    ...
    window.nodefile = {
        ...
        playVideo:function(url) {
            openWindow({
                ...
                max_width: 640,
                max_height: 480,
                url:rootPath + '/dialogs/videoDialog.html',
                args:{
                    source:url
                }
            });
    ...
    window.openWindow = function (options) {
        var defaults = {
            ...
            transparent: true,
            position: 'center',
            parent:null, //父窗口gui.Window
            args: {}//向子页面传递的参数
        };

        angular.extend(defaults, options);
        var newWin = (defaults.parent || gui.Window).open(defaults.url, defaults);

        newWin.on('document-start', function () {
            newWin.window.args = defaults.args;
        });
    ...
    window.nw = {
        ...
        alert: function (options) {
            ...
            typeof options === 'string'? (defaults.content = options) : (angular.extend(defaults, options));
